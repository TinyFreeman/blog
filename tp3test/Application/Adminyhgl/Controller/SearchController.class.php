...
class SearchController extends AdminController {
	...
	public function _initialize(){
		...
		parent::_initialize(5);  //权限id，在config/autority.php中配置
    ...
    public function index_bzcs(){		
		...
		if((empty($map['xing'])&&!empty($map['ming']))or(empty($map['ming'])&&!empty($map['xing']))){
			$this->error('姓名必须写全');
		}
...
//echo '<pre>';print_r($bm);exit;
    ...
    public function index_zxqm(){		
		...
		$where .= empty($map['xing'])?'':' and d.xing=\'' . $map['xing'] . '\' ';
		...
		$list = $bm->field('a.*,b.name t_name,c.username tgs,d.*')->alias('a')->join('left join __ADMIN_T__ b on a.t = b.id ')->join('left join __ADMIN_USER__ c on b.uid = c.id ')->join('left join __ZXQM_USER__ d on a.order_num = d.order_num ')->where($where)->order('a.add_time desc')->limit($Page->firstRow.','.$Page->listRows)->select();
//echo '<pre>';print_r($bm);exit;
		$this->assign('list',$list);
		$this->assign('page',$show); 
	...
	public function index_zwpp(){		
		...
		$where .= empty($map['order_num'])?'':' and a.order_num like \'%' . $map['order_num'] . '%\' ';
		$where .= empty($map['name'])?'':' and a.name=\'' . $map['name'] . '\' ';
...
//echo '<pre>';print_r($bm);exit;
	...
	public function index_zgjm(){		
		...
		$map['order_num'] = urldecode(I('get.order_num'));
		$map['content'] = urldecode(I('get.content'));
		...
		$where .= empty($map['order_num'])?'':' and a.order_num like \'%' . $map['order_num'] . '%\' ';
		$where .= empty($map['content'])?'':' and a.content=\'' . $map['content'] . '\' ';
