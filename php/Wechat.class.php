...
class Wechat
{
    ...
    {       
        $this->token = isset($params['token'])?$params['token']:C('TOKEN');
        $this->encodingAesKey = isset($params['encodingaeskey'])?$params['encodingaeskey']:C('ENCODINGAESKEY');
        $this->appid = isset($params['appid'])?$params['appid']:C('APPID');
        $this->appsecret = isset($params['appsecret'])?$params['appsecret']:C('APPSECRET');
	...
	public function put_log($msg){
		...
		file_put_contents(__DIR__.'/log', $msg);
	}
    ...
    */
     ...
     * @param string $expire 缓存的时间，单位秒
     */
	public function setCache($name,$value,$expire){
		S($name,$value,$expire);
	}
    ...
    public function getUserInfo($openid,$access_token){
        if (!$access_token && !($access_token = $this->checkAuth())) return false;
    ...
    public function sendCustomMessage($data){
        ...
        public function sendCustomMessagejs($data,$atoken){
        if (!$atoken ) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::CUSTOM_SEND_URL.'access_token='.$atoken,self::json_encode($data));
         return $result;
        if ($result)
        {
            $json = json_decode($result,true);
	...
	/**
     * 获取用户openid
	 * 微信文档参考：微信网页开发->微信网页授权
     */
	public function getOpenid(){
		//1 第一步：用户同意授权，获取code
		//2 第二步：通过code换取网页授权access_token
		if(!$arr = $this->getOauthAccessToken()){
			$callback = 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'].$_SERVER['QUERY_STRING'];
			$url = $this->getOauthRedirect($callback, '', 'snsapi_base');		
			Header("Location: $url");
			exit();
		} 
		return $arr['openid'];
...
/*array (
     ...
     function GetOpenid($fee)
    {
        ...
        if (!isset($_GET['code'])){
            ...
            $url = $this->__CreateOauthUrlForCode($baseUrl, $fee);
